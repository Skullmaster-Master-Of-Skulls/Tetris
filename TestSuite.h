...
public:
	static bool runTestSuite()
	{
		...
		//TestSuite::testTetrominoClass(); //why the fuck isn't this working any more?
	...
	static bool testTetrominoClass()
	{
		...
		assert(t.getColour() == TetColour::RED ||
			t.getColour() == TetColour::ORANGE ||
			t.getColour() == TetColour::YELLOW ||
			t.getColour() == TetColour::GREEN ||
			t.getColour() == TetColour::BLUE_LIGHT ||
			t.getColour() == TetColour::BLUE_DARK ||
			t.getColour() == TetColour::PURPLE &&
			"default Tetromino not initialized to valid colour.");
